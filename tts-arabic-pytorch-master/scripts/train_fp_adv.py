...
# ==========================================
        ...
        if torch.isnan(loss_d):
            print("🛡️ Shield: NaN in Discriminator loss. Skipping...")
            continue

        critic.zero_grad()
        loss_d.backward()
        grad_norm_d = torch.nn.utils.clip_grad_norm_(critic.parameters(), 1000.)
        
        if torch.isnan(grad_norm_d):
            print("🛡️ Shield: NaN in Discriminator gradients. Skipping...")
            critic.zero_grad()
            continue
        ...
        if torch.isnan(loss):
            print("🛡️ Shield: NaN in Generator loss. Skipping...")
            optimizer.zero_grad()
            continue
        ...
        if torch.isnan(grad_norm):
            print("🛡️ Shield: NaN in Generator gradients. Skipping...")
            optimizer.zero_grad()
            continue
