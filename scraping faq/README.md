...
# Load the data
with open('psut_faq_rag_optimized.json', 'r', encoding='utf-8') as f:
    data = json.load(f)

# Each document contains:
# - id: Unique identifier
# - text: Combined question + answer for embedding
# - metadata: Rich metadata for filtering and retrieval

for doc in data['documents']:
    # Generate embedding from doc['text']
    # Store in vector DB with doc['id'] and doc['metadata']
    pass
```

## Logging

All execution details are logged to `psut_scraper.log` with timestamps. This includes:
- Successful page loads
- Extracted categories and questions
- Errors and warnings
- Retry attempts

Review this file for debugging or audit purposes.
...
- Processing 4 categories with ~30 total questions

## Support

For issues or questions:
1. Check the log file: `psut_scraper.log`
2. Verify all prerequisites are installed
3. Ensure Firefox browser is installed and updated
...
Please respect the website's terms of service and robots.txt. This scraper includes delays between requests to be respectful to the server. Use responsibly.
